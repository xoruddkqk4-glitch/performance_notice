  ...
  return (
    <main>
      ...
      <section id="schedule" className="content-section">
        ...
        <PublicCalendar assessments={visibleAssessments} view={calendarView} setView={setCalendarView} date={calendarDate} setDate={setCalendarDate} />
      </section>
  ...
  return <main className="dashboard">
    ...
    <section className="dash-main" id="top"><header className="dash-header"><div><p className="eyebrow dark">CLASS MANAGEMENT</p><h1>{tab === "calendar" ? "수행평가 대시보드" : "입력 권한 관리"}</h1></div><span className="admin-badge">회장 관리자</span></header>
      {tab === "calendar" ? <>
        ...
        <PublicCalendar assessments={assessments} view={calendarView} setView={setCalendarView} date={dashboardDate} setDate={setDashboardDate} onDelete={onDelete} onEdit={(item) => { const title = window.prompt("제목 수정", item.title); if (title) onEdit({ ...item, title }); }} />
