...
function Dashboard({ assessments, onAdd, onDelete, onEdit, calendarView, setCalendarView, term, setTerm, className, setClassName, editors, setEditors, onBack, onLogout }: {
  ...
  const eventsBySubject = Object.values(assessments.reduce<Record<string, Assessment[]>>((groups, item) => {
  ...
  })).sort((a, b) => a[0].subject.localeCompare(b[0].subject, "ko"));
  return <main className="dashboard">
    ...
    <section className="dash-main" id="top"><header className="dash-header"><div><p className="eyebrow dark">CLASS MANAGEMENT</p><h1>{tab === "calendar" ? "수행평가 대시보드" : "입력 권한 관리"}</h1></div><span className="admin-badge">회장 관리자</span></header>
      {tab === "calendar" ? <>
        ...
        <section className="all-events-panel"><div className="all-events-heading"><div><p className="eyebrow dark">ALL SCHEDULES</p><h2>{term} · {className} 전체 일정</h2></div><span>{assessments.length}건</span></div>{eventsBySubject.length ? <div className="subject-event-groups">{eventsBySubject.map((subjectEvents) => <section className={`subject-event-group ${subjectEvents[0].color}`} key={subjectEvents[0].subject}><h3>{subjectEvents[0].subject}</h3>{subjectEvents.map((item) => <div className="event-list-row" key={item.id}><time>{item.date}</time><strong>[{item.subject}]{item.title}({item.period})</strong><span>{item.kind}</span></div>)}</section>)}</div> : <p className="empty-schedule">선택한 학기와 학급에 등록된 일정이 없어요.</p>}</section>
