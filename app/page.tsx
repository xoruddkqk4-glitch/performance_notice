...
function FixedCalendar(props: { assessments: Assessment[]; view: "월간" | "2주" | "주간" | "일간" | "과목 카드"; setView: (view: "월간" | "2주" | "주간" | "일간" | "과목 카드") => void; date: Date; setDate: (date: Date) => void; onDelete?: (id: number) => void; onEdit?: (item: Assessment) => void }) {
  ...
  const now = new Date();
  return <><div className="view-tabs public-tabs">{(["과목 카드", "월간", "2주", "주간", "일간"] as const).map((item) => <button key={item} className={view === item ? "selected" : ""} onClick={() => setView(item)}>{item === "과목 카드" ? "카드뷰" : `${item}뷰`}</button>)}<span className="print-actions"><button onClick={() => window.print()}>PDF 출력</button></span></div><section className="calendar-panel public-calendar"><div className="calendar-title"><div className="calendar-nav"><button className="today-button" onClick={() => setDate(new Date())}>오늘</button><button onClick={() => move(-1)}>‹</button><button onClick={() => move(1)}>›</button></div><div><strong>{heading}</strong><span>수행평가 일정</span></div></div>{view === "일간" ? <div className="period-grid">{["1교시", "2교시", "3교시", "4교시", "5교시", "6교시", "7교시"].map((period) => <div className="period-row" key={period}><b>{period}</b><div>{eventsFor(start).filter((event) => event.period.startsWith(period)).map((event) => <div className={`event-dot ${event.color}`} key={event.id}>[{event.subject}]{event.title}({event.period})</div>)}</div></div>)}</div> : <><div className="weekdays">{["일", "월", "화", "수", "목", "금", "토"].map((item) => <span key={item}>{item}</span>)}</div><div className={`month-grid ${view !== "월간" ? "compact-grid" : ""}`}>{cells.map((day) => <div className={`day-cell ${day.toDateString() === now.toDateString() ? "today" : ""}`} key={day.toISOString()}><span>{day.getDate()}</span>{eventsFor(day).map((event) => <div className={`event-dot ${event.color}`} key={event.id}>[{event.subject}]{event.title}({event.period})</div>)}</div>)}</div></>}</section></>;
}
...
function Dashboard({ assessments, onAdd, onDelete, onEdit, calendarView, setCalendarView, term, setTerm, className, setClassName, editors, setEditors, onBack, onLogout }: {
  ...
  const [dashboardDate, setDashboardDate] = useState(new Date(2026, 7, 1)); const [terms, setTerms] = useState(["2026학년도 1학기", "2026학년도 2학기"]); const [classes, setClasses] = useState(["2학년 1반", "2학년 2반", "3학년 2반", "3학년 3반"]);
  ...
  function addEntry(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!entry.subject || !entry.title || !entry.date || !entry.period) return; const toLabel = (value: string) => { const [, m, d] = value.match(/\d{4}-(\d{2})-(\d{2})/) ?? []; return `${Number(m)}월 ${Number(d)}일`; }; const color = ({ "국어":"pink", "영어":"purple", "수학":"blue", "사회":"orange", "과학":"green" } as Record<string, Assessment["color"]>)[entry.subject] ?? "gray"; onAdd({ id: Date.now(), subject: entry.subject, title: entry.title, date: entry.endDate ? `${toLabel(entry.date)} ~ ${toLabel(entry.endDate)}` : toLabel(entry.date), period: entry.endPeriod ? `${entry.period}~${entry.endPeriod}` : entry.period, kind: entry.kind, templateLink: entry.templateLink, color }); setEntry({ subject: "", title: "", date: "", endDate: "", period: "", endPeriod: "", kind: "설명", templateLink: "" }); }
  ...
  return <main className="dashboard">
    ...
    <section className="dash-main" id="top"><header className="dash-header"><div><p className="eyebrow dark">CLASS MANAGEMENT</p><h1>{tab === "calendar" ? "수행평가 대시보드" : "입력 권한 관리"}</h1></div><span className="admin-badge">회장 관리자</span></header>
      {tab === "calendar" ? <>
        <div className="choice-panels"><section><b>학기 선택</b><button className="plus-create" onClick={() => setAddingTerm(!addingTerm)}>＋</button>{addingTerm && <form className="inline-add" onSubmit={(event) => { event.preventDefault(); if (newTerm && !terms.includes(newTerm)) { setTerms([...terms, newTerm]); setTerm(newTerm); } setNewTerm(""); setAddingTerm(false); }}><input autoFocus value={newTerm} onChange={(event) => setNewTerm(event.target.value)} placeholder="새 학기명"/><button>추가</button></form>}<div>{terms.map((item) => <span className="choice-item" key={item}><button className={term === item ? "chosen" : ""} onClick={() => setTerm(item)}>{item}</button><button className="choice-delete" onClick={() => { if (terms.length === 1) return; const next = terms.filter((value) => value !== item); setTerms(next); if (term === item) setTerm(next[0]); }}>×</button></span>)}</div></section><section><b>학급 선택</b><button className="plus-create" onClick={() => setAddingClass(!addingClass)}>＋</button>{addingClass && <form className="inline-add" onSubmit={(event) => { event.preventDefault(); if (newClass && !classes.includes(newClass)) { setClasses([...classes, newClass]); setClassName(newClass); } setNewClass(""); setAddingClass(false); }}><input autoFocus value={newClass} onChange={(event) => setNewClass(event.target.value)} placeholder="새 학급명"/><button>추가</button></form>}<div>{classes.filter((item) => term.includes("1학기") ? item.startsWith("2학년") : item.startsWith("3학년")).map((item) => <span className="choice-item" key={item}><button className={className === item ? "chosen" : ""} onClick={() => setClassName(item)}>{item}</button><button className="choice-delete" onClick={() => { if (classes.length === 1) return; const next = classes.filter((value) => value !== item); setClasses(next); if (className === item) setClassName(next.find((value) => term.includes("1학기") ? value.startsWith("2학년") : value.startsWith("3학년")) ?? next[0]); }}>×</button></span>)}</div></section></div>
        <section className="entry-panel"><div><p className="eyebrow dark">SCHEDULE INPUT</p><h2>일정 입력 및 공유</h2><p>학생용 공유 페이지: <code>{shareLink}</code></p><button className="copy-link" onClick={() => navigator.clipboard?.writeText(shareLink)}>링크 복사</button><img className="share-qr" src="/student-share-qr.png" alt="학생용 공유 페이지 QR 코드" /></div><form className="entry-form" onSubmit={addEntry}><input placeholder="과목" value={entry.subject} onChange={(e) => setEntry({ ...entry, subject: e.target.value })}/><input placeholder="제목" value={entry.title} onChange={(e) => setEntry({ ...entry, title: e.target.value })}/><label>시작일<input type="date" value={entry.date} onChange={(e) => setEntry({ ...entry, date: e.target.value })}/></label><label>종료일 (기간 선택)<input type="date" value={entry.endDate} min={entry.date} onChange={(e) => setEntry({ ...entry, endDate: e.target.value })}/></label><label>교시<select value={entry.period} onChange={(e) => setEntry({ ...entry, period: e.target.value })}><option value="">선택</option>{["1교시", "2교시", "3교시", "4교시", "5교시", "6교시", "7교시", "제출"].map((item) => <option key={item}>{item}</option>)}</select></label><input placeholder="설명" value={entry.kind} onChange={(e) => setEntry({ ...entry, kind: e.target.value })}/><input placeholder="양식 링크 (선택)" value={entry.templateLink} onChange={(e) => setEntry({ ...entry, templateLink: e.target.value })}/><button>일정 추가</button></form></section>
        <FixedCalendar assessments={assessments} view={calendarView} setView={setCalendarView} date={dashboardDate} setDate={setDashboardDate} onDelete={onDelete} onEdit={(item) => { const title = window.prompt("제목 수정", item.title); if (title) onEdit({ ...item, title }); }} />
