  ...
  return <main className="dashboard">
    ...
    <section className="dash-main" id="top"><header className="dash-header"><div><p className="eyebrow dark">CLASS MANAGEMENT</p><h1>{tab === "calendar" ? "수행평가 대시보드" : "입력 권한 관리"}</h1></div><span className="admin-badge">회장 관리자</span></header>
      {tab === "calendar" ? <>
        ...
        <section className="share-panel"><p className="eyebrow dark">SCHEDULE SHARE</p><h2>일정 공유</h2><p>학생용 공유 페이지: <code>{shareLink}</code></p><button className="copy-link" onClick={() => navigator.clipboard?.writeText(shareLink)}>링크 복사</button><img className="share-qr" src="/student-share-qr.png" alt="학생용 공유 페이지 QR 코드" /></section>
